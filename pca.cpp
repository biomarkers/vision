...
static Mat toGrayscale(InputArray _src) {
    ...
    if(src.channels() != 1) {
        CV_Error(0, "Only Matrices with one channel are supported");
    }
...
static void onTrackbar(int pos, void* ptr)
{
    ...
    p->pca = PCA(p->data, cv::Mat(), CV_PCA_DATA_AS_ROW, 1);
...
int main(int argc, char** argv)
{
    ...
    if(images.size() <= 1) {
        ...
        CV_Error(0, error_message);
    }

    std::cout << "About to reformat images......\n\n\n\n" << endl;

    for(int c = 0; c < images.size(); c++)
    {
	cout << c << " " << images[c].size().height << images[c].size().width << endl;
    }
